...
A highly robust, chronological insurance benefits calculation engine written in Node.js. It automatically parses insurance policies and calculates payouts across a chronological sequence of claims, applying complex combinations of waiting periods, diagnosis exclusions, annual limits, sub-limits, deductibles, and multi-tier copays (both percentage and flat/fixed copays).
...
3. **Robust Output Schema**: Returns structured JSON data for each processed expense detailing exact payouts, remaining deductibles, remaining annual and visit limits, and human-readable audit reasons.
4. **Rich Verification Suite**: Features 14 comprehensive Jest unit tests covering every standard, boundary, and defensive edge case.
...
- **`src/`**
  - [`calculator.js`](./src/calculator.js): The core benefits evaluation pipeline and limit state manager.
  - [`run.js`](./src/run.js): The executable runner script printing colorized CLI summaries and generating `expected_outputs.json`.
- **`tests/`**
  - [`calculator.test.js`](./tests/calculator.test.js): 14 Jest assertions covering all complex business rules and validations.
- **`output/`**
...
### 1. Installation
Install Jest testing framework and other dependencies:
```bash
npm install
```
