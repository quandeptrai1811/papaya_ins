...
interface DiagCostBarProps {
  claims: Claim[];
  activeDiag: string | null;
  onSelect: (diag: string | null) => void;
}

export default function DiagCostBar({ claims, activeDiag, onSelect }: DiagCostBarProps) {
  const data = useMemo(() => {
    ...
    <div className="chart-card">
      ...
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={data} layout="vertical" margin={{ left: 20 }}>
          ...
          />
          <Bar dataKey="total" radius={[0, 4, 4, 0]} cursor="pointer" onClick={(d: any) => onSelect(activeDiag === d.full ? null : d.full)}>
            {data.map(entry => (
