...
export default function DiagFreqBar({ claims, activeDiag, onSelect }) {
  const data = useMemo(() => {
    const map = {};
    ...
    <div className="chart-card">
      ...
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={data} layout="vertical" margin={{ left: 20 }}>
          ...
          />
          <Bar dataKey="count" radius={[0, 4, 4, 0]} cursor="pointer" onClick={(d) => onSelect(activeDiag === d.full ? null : d.full)}>
            {data.map(entry => (
